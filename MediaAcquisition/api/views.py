...
#todo: If failure occurs on FTP storage, delete new entry in database and return error response
...
def get_audio(request, link):
    try:
        globalController = domainController()
        url = globalController.get_audio(link)
        if url == None:
            return HttpResponseNotFound('There was an error retrieving the track URL')
    ...
    try:
        if request.method == 'POST':
            if not request.FILES['mp3file'] is None:
                ...
                return HttpResponse('Successfully uploaded ' + filename + '! Audio: ' + str(audio_id))
        else:
    ...
    try:
        ...
        if select == 'YT_':
            ...
            try:
                ...
                return HttpResponse('Song not found in file system')
